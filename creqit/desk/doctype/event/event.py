...
# from creqit.contacts.doctype.contact.contact import get_default_contact
...
from creqit.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from creqit.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			creqit.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			creqit.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		communications = creqit.get_all(
			"Communication", dict(reference_doctype=self.doctype, reference_name=self.name)
		)
		if communications:
			for communication in communications:
				creqit.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				if comms := creqit.get_all("Communication", filters=filters, fields=["name"], distinct=True):
					for comm in comms:
						communication = creqit.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = creqit.get_meta(participant.reference_doctype)
	...
	def create_communication(self, participant):
		communication = creqit.new_doc("Communication")
		self.update_communication(participant, communication)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = creqit.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				creqit.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@creqit.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = creqit.get_doc(reference_doctype, reference_docname)
	...
	comms = creqit.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = creqit.get_doc("Communication", comm.name).delete()
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={creqit.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			creqit.set_user_lang(user.name, user.language)
			...
			creqit.sendmail(
				...
				subject=creqit._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[creqit._("Events in Today's Calendar"), "blue"],
			)


@creqit.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[creqit._dict]:
	if not user:
		user = creqit.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	...
	if participations:
		for participation in participations:
			if delete_event:
				creqit.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = creqit.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					creqit.db.delete("Event", {"name": participation.parent})
					creqit.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = creqit.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			creqit.db.set_value("Event", event.name, "status", "Closed")
